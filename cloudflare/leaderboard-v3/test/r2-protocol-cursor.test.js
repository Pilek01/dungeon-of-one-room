...
test("released client accepts the activated candidate bootstrap without launcher patching", async () => {
...
test("R2 client accepts the activated manifest and released save hashes while rejecting unknown hashes", () => {
  ...
  assert.equal(
    protocol.supportsBoundedProcClaims("sha256:76514cf9e5c89079571a5be117ce84f949d7a3f5ed441d973adc05c95c6dde3c"),
    false
  );
  assert.equal(
...
test("R2 client fails closed on malformed nested projections and unknown response kinds", () => {
  ...
  assert.deepEqual(
    protocol.validateMutationResponse(withRankIntegrity).metaState.rankIntegrity.reasonCodes,
  ...
  assert.throws(
    ...
    /PROTOCOL_PROJECTION_INVALID:rankIntegrity/u
  );
