...
import {
  ...
  V08_META_1_SPECIAL_ROOM_ROTATION_PREVIOUS_PRODUCTION_RELEASE_DESCRIPTOR,
...
test("production promotion activates the exact candidate and retains its immediate predecessor", () => {
  ...
  assert.ok(observerProfilePrevious, "the previous Observer-profile descriptor must be retained");
  assert.equal(observerProfilePrevious.rulesetHash, PREVIOUS_OBSERVER_PROFILE_HASH);
  ...
  assert.ok(protocol.SUPPORTED_RULESET_HASHES.includes(PREVIOUS_MERCHANT_FAVOR_HASH));
...
test("room-transition repair retains the immediately previous Chronicle production hash", () => {
  ...
  assert.deepEqual(
    ...
    releases.V08_META_1_ROOM_NAVIGATION_PREVIOUS_PRODUCTION_RELEASE_DESCRIPTOR.capabilities
  );
...
test("local candidate is promoted to production with its exact capability contract", async () => {
  ...
  assert.equal(V08_META_1_LOCAL_RELEASE_DESCRIPTOR.capabilities.roomEliteBudgetByType, "v2");
  ...
  assert.equal(V08_META_1_PRODUCTION_RELEASE_DESCRIPTOR.capabilities.specialRoomRotation, "v1");
  ...
  const roomNavigationPrevious = registry.resolve({
    rulesetId: "v08-meta-1",
    rulesetHash: PREVIOUS_ROOM_NAVIGATION_HASH,
    environment: "production",
    lifecycle: "ranked"
  });
  assert.equal(roomNavigationPrevious.rulesetHash, PREVIOUS_ROOM_NAVIGATION_HASH);
  assert.equal(roomNavigationPrevious.capabilities.campaignChronicle, "v1");
  assert.equal(roomNavigationPrevious.capabilities.merchantExitBarrier, undefined);
