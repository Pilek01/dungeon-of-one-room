(() => {
  ...
  function isBlocked(tile, enemy, enemies, chests, pits, inBounds) {
    if (!inBounds(tile.x, tile.y)) return true;
    if (pits.has(`${tile.x},${tile.y}`)) return true;
  ...
  function decidePlan(input) {
    ...
    const pits = new Set((Array.isArray(input.pits) ? input.pits : []).map((pit) => `${pit.x},${pit.y}`));
    ...
    for (const dir of DIRECTIONS) {
      ...
      } else if (isBlocked(tile, enemy, enemies, chests, pits, input.inBounds)) {
        continue;
      }
    ...
    if (
      ...
      if (
        !isBlocked(memoryTile, enemy, enemies, chests, pits, input.inBounds) &&
