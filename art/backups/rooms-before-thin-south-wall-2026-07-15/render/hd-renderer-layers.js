(function attachHDRendererLayers(root, factory) {
  ...
  const api = factory(vfxApi, lightingApi, statusApi);
...
})(typeof window !== "undefined" ? window : null, function createHDRendererLayersApi(vfxApi, lightingApi, statusApi) {
  "use strict";
  ...
  const ENEMY_ROSTER = Object.freeze(["slime", "skeleton", "brute", "acolyte", "skitter", "totem", "otter"]);
  ...
  function selectStandardRoomBackground(snapshot) {
    ...
    const variantCounts = { descent: 3, corruption: 3, abyss: 3 };
  ...
  function drawDecalsLayer(context, snapshot, assets) {
    ...
    if (visual.bossRoom === true) {
      ...
      drawCentered(context, assets, "object.boss.relief.north", 4, 0, TILE_SIZE, TILE_SIZE);
      drawCentered(context, assets, "object.boss.relief.south", 4, GRID_SIZE - 1, TILE_SIZE, TILE_SIZE);
    }
  }
  ...
  function drawHazardsLayer(context, snapshot, assets) {
    ...
    for (const mine of mines) {
      if (!mine) continue;
      const state = mine.armed === false ? "unarmed" : "armed";
  ...
  function drawCentered(context, assets, key, gridX, gridY, width, height) {
    if (!isGridCoordinate(gridX) || !isGridCoordinate(gridY)) return false;
    const x = gridX * TILE_SIZE + TILE_SIZE * 0.5 - width * 0.5;
    const y = gridY * TILE_SIZE + TILE_SIZE * 0.5 - height * 0.5;
    return drawAsset(context, assets, key, x, y, width, height);
  }
  ...
  function drawObjectsLayer(context, snapshot, assets) {
    ...
    for (const chest of chests) {
      if (chest && chest.type !== "otter_red" && !chest.opened) {
        drawGridAsset(context, assets, "object.common.chest.normal", chest.x, chest.y);
      }
    }
    ...
    if (visual.portal && visual.roomCleared === true) {
      const kind = ["vault", "forge", "otter"].includes(visual.portal.kind)
        ? visual.portal.kind
        : ["vault", "forge", "otter"].includes(visual.roomType)
          ? visual.roomType
          : "default";
      ...
      if (separatedAvailable || commonSeparatedAvailable) {
        ...
        drawBottomCentered(context, assets, selectedFrame, visual.portal.x, visual.portal.y, TILE_SIZE * 2, TILE_SIZE * 2);
        drawBottomCentered(context, assets, selectedSwirl, visual.portal.x, visual.portal.y, TILE_SIZE * 2, TILE_SIZE * 2);
      } else {
        drawBottomCenteredFallback(
          context,
          assets,
  ...
  function selectEnemyDirection(enemy) {
    if (enemy.type === "totem") return "base";
    return PLAYER_DIRECTIONS.includes(enemy.facing) ? enemy.facing : "south";
  }

  function selectEnemyVisual(snapshot, enemy) {
    const visual = snapshot && typeof snapshot === "object" ? snapshot : {};
    const actor = enemy && typeof enemy === "object" ? enemy : {};
    if (!ENEMY_ROSTER.includes(actor.type)) return Object.freeze({ diagnostic: true });
    ...
    else if (actor.type === "acolyte" && (actor.aiming === true || (Number(actor.castFlash) || 0) > 0)) clip = "attack";
    else if (actor.type === "brute" && (actor.slamAiming === true || actor.rests === true)) clip = "attack";
    ...
    if (clip === "idle" || clip === "move") {
      ...
      frame = (Math.floor(elapsed / (1000 / fps)) % frameCount) + 1;
    } else if (clip === "hit") {
      frame = (Number(actor.hitFlash) || 0) > 60 ? 1 : 2;
    } else if (clip === "cast" || ((clip === "attack") && (Number(actor.castFlash) || 0) > 0)) {
      const remaining = Math.max(0, Math.min(140, Number(actor.castFlash) || 0));
      frame = Math.min(frameCount, 1 + Math.floor((140 - remaining) / (140 / frameCount)));
    } else if (clip === "attack") {
      frame = Math.min(frameCount, 1 + Math.max(0, Math.floor(Number(actor.telegraphAge) || 0)));
    }
    const key = `enemy.${actor.type}.${direction}.${clip}.${String(frame).padStart(2, "0")}`;
    return Object.freeze({ type: actor.type, direction, clip, frame, key });
  }
  ...
  const BOSS_PROFILES = Object.freeze({
    ...
    warden2: Object.freeze({ key: "warden.phase2", action: "cast", renderSize: 192 })
  });
  ...
  function selectBossVisual(snapshot, enemy) {
    ...
    return Object.freeze({
      ...
      key: `boss.${profile.key}.${direction}.${clip}.${String(frame).padStart(2, "0")}`
    });
  }
  ...
  function drawEnemyCrests(context, enemy, assets, options = {}) {
    ...
    const positions = [
      { x: Math.round(left - 18), y: Math.round(top + 6) },
      { x: Math.round(right + 2), y: Math.round(top + 6) }
    ];
    const drawn = [];
    for (const [index, descriptor] of statusApi.selectEnemyCrests(enemy).entries()) {
      const position = positions[index];
      if (!position || !getAsset(assets, descriptor.key)) continue;
      drawAsset(context, assets, descriptor.key, position.x, position.y, 16, 16);
      drawn.push(Object.freeze({ id: descriptor.id, ...position, width: 16, height: 16 }));
    }
  ...
  function drawTelegraphsLayer(context, snapshot) {
    if (vfxApi && typeof vfxApi.drawTelegraphs === "function") vfxApi.drawTelegraphs(context, snapshot);
  }

  function drawVfxLayer(context, snapshot) {
    if (vfxApi && typeof vfxApi.drawVfx === "function") vfxApi.drawVfx(context, snapshot);
  }
