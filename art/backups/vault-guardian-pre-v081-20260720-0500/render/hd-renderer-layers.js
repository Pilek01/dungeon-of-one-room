(function attachHDRendererLayers(root, factory) {
  ...
  const ENEMY_RENDER_PROFILES = Object.freeze({
    ...
    bulwark: Object.freeze({ renderSize: 88 })
  });
  ...
  function drawObjectsLayer(context, snapshot, assets) {
    ...
    const drawCrossroadsChoiceLabel = (chest) => {
      ...
      const panelX = TILE_SIZE * (isPower ? 0.3 : 4.7);
      const panelY = TILE_SIZE * 1.35;
  ...
  function selectEnemyVisual(snapshot, enemy) {
    ...
    if (clip === "idle") {
      ...
      if (["riftweaver", "bulwark"].includes(actor.type)) {
        frame = (Math.floor(elapsed / 110) % frameCount) + 1;
      } else {
  ...
  function drawEnemiesLayer(context, snapshot, assets) {
    ...
    for (const enemy of enemies) {
      ...
      if (drawn && enemy.type === "bulwark") {
        const guardFrame = (Math.floor(Math.max(0, Number(snapshot && snapshot.nowMs) || 0) / 150) % 4) + 1;
        const guardKey = `asset.vfx.expansion.bulwark.guard.${selection.direction}.guard_${String(guardFrame).padStart(2, "0")}`;
        drawAsset(context, assets, guardKey, drawX, drawY, renderSize, renderSize);
      }
    ...
    for (const enemy of Array.isArray(snapshot && snapshot.enemies) ? snapshot.enemies : []) {
      ...
      if (enemy.riftAiming === true) {
        drawCentered(
          ...
          enemy.riftTargetX, enemy.riftTargetY, 192, 192
        );
      }
      ...
      if (enemy.latticeAiming === true) {
        const rows = Array.isArray(enemy.latticeRows) ? enemy.latticeRows : [];
        const columns = Array.isArray(enemy.latticeColumns) ? enemy.latticeColumns : [];
