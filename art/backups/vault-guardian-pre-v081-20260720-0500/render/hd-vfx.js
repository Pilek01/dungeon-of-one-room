(function attachHDVfx(root, factory) {
  ...
  function collectTelegraphCommands(snapshot, options = {}) {
    ...
    for (const sigil of Array.isArray(visual.doomSigils) ? visual.doomSigils : []) {
      if (!sigil) continue;
      const urgency = (Number(sigil.fuseTurns) || 0) <= 1 ? 1 : 0.55;
      const tiles = [
        { x: sigil.x, y: sigil.y }, { x: sigil.x + 1, y: sigil.y }, { x: sigil.x - 1, y: sigil.y },
        { x: sigil.x, y: sigil.y + 1 }, { x: sigil.x, y: sigil.y - 1 }
      ];
      for (const tile of tiles) {
        if (tile.x < 0 || tile.x >= GRID_SIZE || tile.y < 0 || tile.y >= GRID_SIZE) continue;
        commands.push({ kind: "doom-sigil-area", x: tile.x, y: tile.y, color: "#e24dff", alpha: (0.1 + pulse * 0.18) * urgency * profile.motionScale });
      }
    }
    ...
    for (const enemy of Array.isArray(visual.enemies) ? visual.enemies : []) {
      ...
      if (enemy.riftAiming === true) {
        for (const tile of areaTiles(enemy.riftTargetX, enemy.riftTargetY, 1)) {
          commands.push({ kind: "rift-area", x: tile.x, y: tile.y, color: "#9d4dff", alpha: (0.12 + pulse * 0.2) * profile.motionScale });
        }
        continue;
      }
      ...
      if (enemy.latticeAiming === true) {
        for (const row of Array.isArray(enemy.latticeRows) ? enemy.latticeRows : []) {
          for (let x = 1; x < GRID_SIZE - 1; x += 1) commands.push({ kind: "lattice-area", x, y: clamp(row, 1, 7), color: "#a96cff", alpha: (0.1 + pulse * 0.18) * profile.motionScale });
        }
        for (const column of Array.isArray(enemy.latticeColumns) ? enemy.latticeColumns : []) {
          for (let y = 1; y < GRID_SIZE - 1; y += 1) commands.push({ kind: "lattice-area", x: clamp(column, 1, 7), y, color: "#a96cff", alpha: (0.1 + pulse * 0.18) * profile.motionScale });
        }
        continue;
      }
      if (enemy.voidStepAiming === true) {
        for (const tile of areaTiles(enemy.voidStepOriginX, enemy.voidStepOriginY, 1)) {
          commands.push({ kind: "voidstep-area", x: tile.x, y: tile.y, color: "#7651ff", alpha: (0.11 + pulse * 0.19) * profile.motionScale });
        }
        continue;
      }
      if (enemy.soulChainAiming === true) {
        for (const tile of Array.isArray(enemy.soulChainTiles) ? enemy.soulChainTiles : []) {
          commands.push({ kind: "soul-chain-area", x: clamp(tile.x, 0, 8), y: clamp(tile.y, 0, 8), color: "#ef7bff", alpha: (0.13 + pulse * 0.2) * profile.motionScale });
        }
        continue;
      }
  ...
  function assetCommand(key, gridX, gridY, size = TILE_SIZE, alpha = 1) {
    return {
      kind: "asset-vfx",
      key,
      x: gridX * TILE_SIZE + TILE_SIZE * 0.5 - size * 0.5,
      y: gridY * TILE_SIZE + TILE_SIZE * 0.5 - size * 0.5,
      width: size,
      height: size,
      alpha
    };
  ...
  function collectVisualEventCommands(visual, profile) {
    ...
    for (const event of Array.isArray(visual.visualEvents) ? visual.visualEvents : []) {
      ...
      if (event.kind === "riftweaver_rift_detonate") {
        frameCount = 8; prefix = "asset.vfx.expansion.riftweaver.spatial_rift.detonation.detonation"; size = 192;
      ...
      commands.push(assetCommand(`${prefix}_${suffix}`, event.x, event.y, size, alpha));
    }
    return commands;
  }
  ...
  function drawTelegraphs(context, snapshot, options = runtimeOptions()) {
    withContext(context, () => {
