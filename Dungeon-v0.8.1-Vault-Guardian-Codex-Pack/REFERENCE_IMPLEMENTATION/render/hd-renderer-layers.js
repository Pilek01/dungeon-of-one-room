(function attachHDRendererLayers(root, factory) {
  ...
  function drawFlameVentMarker(context, vent, nowMs) {
    ...
    for (let index = 0; index < fuse; index += 1) context.fillRect(px + 22 + index * 8, py + 50, 5, 3);
  }

  function drawFrostRuneMarker(context, rune, nowMs) {
    if (!context || !rune || rune.spent === true || typeof context.fillRect !== "function") return;
  ...
  function drawDoomSigilMarker(context, sigil, nowMs) {
  ...
  function drawHazardsLayer(context, snapshot, assets) {
    ...
    for (const vent of flameVents) drawFlameVentMarker(context, vent, nowMs);
    for (const rune of frostRunes) drawFrostRuneMarker(context, rune, nowMs);
    for (const sigil of doomSigils) drawDoomSigilMarker(context, sigil, nowMs);
  }
  ...
  function drawObjectsLayer(context, snapshot, assets) {
    ...
    const drawVaultChestLock = (chest) => {
      ...
      const pulse = 0.5 + Math.sin((Math.max(0, Number(visual.nowMs) || 0) + chest.x * 73 + chest.y * 37) / 135) * 0.5;
      context.save();
  ...
  function selectEnemyVisual(snapshot, enemy) {
    ...
    const renderType = ENEMY_ROSTER.includes(actor.renderType) ? actor.renderType : actor.type;
    ...
    else if (actor.type === "bulwark" && (actor.bulwarkBashAiming === true || actor.rests === true)) clip = "attack";
  ...
  function drawEnemiesLayer(context, snapshot, assets) {
    ...
    for (const enemy of enemies) {
      ...
      if (drawn && enemy.type === "riftweaver" && typeof context.fillRect === "function") {
        context.globalAlpha = 0.22;
        context.fillStyle = "#9c4cff";
        context.fillRect(drawX + renderSize * 0.18, drawY + renderSize * 0.18, renderSize * 0.64, renderSize * 0.64);
        context.globalAlpha = 1;
      } else if (drawn && enemy.type === "bulwark" && typeof context.strokeRect === "function") {
        context.globalAlpha = 0.85;
        context.strokeStyle = "#b9d5e5";
        context.lineWidth = 3;
        context.strokeRect(drawX + renderSize * 0.18, drawY + renderSize * 0.2, renderSize * 0.64, renderSize * 0.64);
        context.globalAlpha = 1;
      }
