...
async function captureShot(browser, baseUrl, shot, report, viewport = LANDSCAPE, suffix = "") {
  ...
  try {
    ...
    if (shot.id === "16-records") {
      await page.locator(".record-archive-inspect-button").first().tap();
      await page.locator(".record-archive-detail").waitFor({ state: "visible", timeout: 30000 });
      const backBox = await page.locator(".record-archive-back").boundingBox();
      ...
      await page.locator(".record-archive-back").tap();
      await page.locator(".record-archive-list").waitFor({ state: "visible", timeout: 30000 });
    }
