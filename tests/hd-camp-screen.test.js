...
assert.match(game, /relic_ward:/);
assert.match(game, /relic_appraisal:/);
assert.match(game, /camp-revamp-key camp-revamp-icon/);
assert.match(game, /camp-revamp-content/);
assert.match(game, /camp-revamp-guide[\s\S]*Left \/ Right tabs \| Up \/ Down select \| Enter/);
...
assert.match(css, /\.camp-revamp-grid\s*\{[\s\S]*grid-template-columns:\s*repeat\(2,[\s\S]*grid-template-rows:\s*repeat\(6,[\s\S]*overflow:\s*hidden/);
