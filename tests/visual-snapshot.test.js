...
test("clones nested render structures without cloning arbitrary fields", () => {
  const source = {
    ...
    enemies: [{
      ...
      voidAegisShield: 40,
      latticeAiming: true,
      latticeCastsRemaining: 0,
      latticePatterns: [
        { rows: [4], columns: [3], turnsRemaining: 1, sequenceIndex: 1 },
        { rows: [5], columns: [6], turnsRemaining: 2, sequenceIndex: 2 }
      ],
      brain: { target: "player" }
    }],
  ...
  snapshot.enemies[0].latticePatterns[0].rows[0] = 99;
  ...
  assert.strictEqual(source.enemies[0].latticePatterns[0].rows[0], 4);
  assert.deepStrictEqual(snapshot.enemies[0].latticePatterns.map((pattern) => pattern.turnsRemaining), [1, 2]);
